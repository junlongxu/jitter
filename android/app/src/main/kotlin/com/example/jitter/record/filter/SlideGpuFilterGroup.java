package com.example.jitter.record.filter;
...
import com.example.jitter.record.other.MagicFilterFactory;
import com.example.jitter.record.other.MagicFilterType;
...
public class SlideGpuFilterGroup {
    ...
    public SlideGpuFilterGroup() {
        initFilter();
        scroller = new Scroller(Utils.mContext);
    }
    ...
    public void onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                locked = true;
                downX = -1;
                if (offset > MainActivity.Companion.getScreenWidth() / 3) {
                    scroller.startScroll(offset, 0, MainActivity.Companion.getScreenWidth() - offset, 0, 100 * (1 - offset / MainActivity.Companion.getScreenWidth()));
                    needSwitch = true;
                } else {
                    scroller.startScroll(offset, 0, -offset, 0, 100 * (offset / MainActivity.Companion.getScreenWidth()));
                    needSwitch = false;
                }
                break;
        }
